...
y_classification = pd.cut(y, bins=[-float('inf'), -0.1, 0.1, float('inf')], labels=['Negative', 'Neutral', 'Positive'], right=True)
...
# add to the data
data['seat_traveller'] = X['seat_traveller']
data['country_route'] = X['country_route']
data['day_of_week'] = X['day_of_week']
data['month'] = X['month']
data['year'] = pd.to_datetime(data['date']).dt.year
data['avg_rating_country'] = X['avg_rating_country']
data['avg_rating_route'] = X['avg_rating_route']
...
X_reg = data[['recommended', 'stars'] + numeric_features + ['seat_traveller', 'country_route']]
y_reg = data['SentimentScore']

# One-hot encode categorical features for regression
X_reg = pd.get_dummies(X_reg, columns=['seat_traveller', 'country_route'], drop_first=True)
